...
def my_datagen(path,y0,size=16,width = 299):
    ...
    while True:
        ...
        while len(X)<size:
                ...
                img=cv2.resize(cv2.imread(path[v[i]]), (299,299))
...
model.load_weights('../models/all23new.best.h5')
#model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['acc'])

checkpointer = ModelCheckpoint(filepath='../models/all23new.best.h5', verbose=1, monitor='val_loss',mode='min',
                               save_best_only=True)
model.compile(optimizer=adam, loss='categorical_crossentropy', metrics=[acc])
...
for i in tqdm(range(n)):
    X_test[i] = cv2.resize(cv2.imread('../test_a2/{0}'.format(df_load['image_id'][i])), (width, width))
...
df_load.head()
df_load.to_csv('../result/{}_0331bx.csv', header=None, index=False)
